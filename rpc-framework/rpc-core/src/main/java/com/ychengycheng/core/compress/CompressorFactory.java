package com.ychengycheng.core.compress;

import com.ychengycheng.config.wrapper.ObjectWrapper;
import com.ychengycheng.core.compress.impl.GzipCompressor;
...
public class CompressorFactory {
    ...
    private final static Map<String, ObjectWrapper<Compressor>> COMPRESSOR_CACHE =
            new ConcurrentHashMap<>(
            8);
    private final static Map<Byte, ObjectWrapper<Compressor>> COMPRESSOR_CACHE_CODE =
            new ConcurrentHashMap<>(
            8);
    ...
    public static ObjectWrapper<Compressor> getCompressor(String compressorType) {
        ...
        if (compressorObjectWrapper == null) {
            log.warn("Compress protocol [{}]  NOT FOUND , using GZIP instead", compressorType);
    ...
    public static ObjectWrapper<Compressor> getCompressor(byte serializeCode) {
        ...
        if ((compressorObjectWrapper == null)) {
            log.warn("Compress protocol code [{}]  NOT FOUND , using GZIP instead", serializeCode);
            return COMPRESSOR_CACHE.get("gzip");
        }
        return compressorObjectWrapper;
    }
